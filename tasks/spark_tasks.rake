...
namespace :spark do		
  ...
  task :fixtures => [:environment,'db:reset'] do
    ...
    ActiveRecord::Base.establish_connection(RAILS_ENV.to_sym)
		SPARK_FIXTURES_ROOT = File.join(SPARK_ROOT,RAILS_ENV,'fixtures')
	...
	task :scripts => [:environment,'db:reset'] do
		include Spark::Mixin
		Dir.glob(File.join(RAILS_ROOT, "db/spark/#{RAILS_ENV}/scripts", '*.{rb}')).each { |script_file| require script_file }
	end
end
